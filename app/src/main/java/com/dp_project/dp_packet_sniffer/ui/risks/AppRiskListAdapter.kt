...
class AppRiskListAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (convertView == null) {
            itemView =
                LayoutInflater.from(context).inflate(R.layout.app_list_risk_item, parent, false)
        }
        ...
        if (itemView != null) {
            ...
            if (currentItem.value[0]) {
                itemView.findViewById<TextView>(R.id.encryptionWarningTextView).visibility =
                    View.VISIBLE
            }
            if (currentItem.value[1]) {
                itemView.findViewById<TextView>(R.id.protocolWarningTextView).visibility =
                    View.VISIBLE
            }
            if (currentItem.value[2]) {
                itemView.findViewById<TextView>(R.id.countryWarningTextView).visibility =
                    View.VISIBLE
            }
            if (currentItem.value[3]) {
                itemView.findViewById<TextView>(R.id.trafficWarningTextView).visibility =
                    View.VISIBLE
            }
        }
        return itemView!!
